...
class HMChaussureSpider(scrapy.Spider):
    name = 'hmspiderChaussure'
    ...
    def parse(self, response):
        ...
        for chaussure in chaussures:
            if '/homme/' in chaussure:
                yield response.follow(chaussure, callback=self.parse_chaussures, meta={'gender': 'men'})
            elif '/femme/' in chaussure:
                yield response.follow(chaussure, callback=self.parse_chaussures, meta={'gender': 'women'})


    def parse_chaussures(self, response):
        gender = response.meta.get('gender')

        chaussures_article = response.xpath('//*[@id="products-listing-section"]/ul/li/section/article')
        prices = [] 
        ...
        #             "VALUES (%s, %s, %s)")
        for article in chaussures_article:
        ...
        if next_page:
            yield response.follow(next_page, callback=self.parse_chaussures, meta={'gender': gender})
    ...
    def closed(self, reason):
        ...
        <body>
            ...
            <div class="container mt-5">
                <h3 class="text-center mt-4 mb-4">Chaussure Statistics</h3>
        ...
        with open('template/chaussure_prices.html', 'w', encoding='utf-8') as f:
