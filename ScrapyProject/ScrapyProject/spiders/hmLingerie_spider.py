...
class HMLingerieSpider(scrapy.Spider):
    name = 'hmspiderLingerie'
    ...
    def parse(self, response):
        lingerie=[
            response.css('a[href*="/fr_fr/homme/catalogue-par-produit/sous-vetements.html"]::attr(href)').get(),
            response.css('a[href*="/fr_fr/femme/catalogue-par-produit/lingerie.html"]::attr(href)').get(),
        ]
        for ling in lingerie:
            if '/homme/' in ling:
                yield response.follow(ling, callback=self.parse_lingerie, meta={'gender': 'men'})
            elif '/femme/' in ling:
                yield response.follow(ling, callback=self.parse_lingerie, meta={'gender': 'women'})


    def parse_lingerie(self, response):
        gender = response.meta.get('gender')

        lingerie_article = response.xpath('//*[@id="products-listing-section"]/ul/li/section/article')
        prices = [] 
        ...
        #             "VALUES (%s, %s, %s)")
        for article in lingerie_article:
        ...
        if next_page:
            yield response.follow(next_page, callback=self.parse_lingerie, meta={'gender': gender})
    ...
    def closed(self, reason):
        ...
        <body>
            ...
            <div class="container mt-5">
                <h3 class="text-center mt-4 mb-4">Lingerie Statistics</h3>
        ...
        with open('template/lingerie_prices.html', 'w', encoding='utf-8') as f:
